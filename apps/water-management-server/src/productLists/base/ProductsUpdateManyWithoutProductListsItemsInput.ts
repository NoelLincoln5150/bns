/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ProductsWhereUniqueInput } from "../../products/base/ProductsWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class ProductsUpdateManyWithoutProductListsItemsInput {
  @Field(() => [ProductsWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ProductsWhereUniqueInput],
  })
  connect?: Array<ProductsWhereUniqueInput>;

  @Field(() => [ProductsWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ProductsWhereUniqueInput],
  })
  disconnect?: Array<ProductsWhereUniqueInput>;

  @Field(() => [ProductsWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ProductsWhereUniqueInput],
  })
  set?: Array<ProductsWhereUniqueInput>;
}

export { ProductsUpdateManyWithoutProductListsItemsInput as ProductsUpdateManyWithoutProductListsItemsInput };
