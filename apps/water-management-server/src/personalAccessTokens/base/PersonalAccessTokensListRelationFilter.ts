/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { PersonalAccessTokensWhereInput } from "./PersonalAccessTokensWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class PersonalAccessTokensListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => PersonalAccessTokensWhereInput,
  })
  @ValidateNested()
  @Type(() => PersonalAccessTokensWhereInput)
  @IsOptional()
  @Field(() => PersonalAccessTokensWhereInput, {
    nullable: true,
  })
  every?: PersonalAccessTokensWhereInput;

  @ApiProperty({
    required: false,
    type: () => PersonalAccessTokensWhereInput,
  })
  @ValidateNested()
  @Type(() => PersonalAccessTokensWhereInput)
  @IsOptional()
  @Field(() => PersonalAccessTokensWhereInput, {
    nullable: true,
  })
  some?: PersonalAccessTokensWhereInput;

  @ApiProperty({
    required: false,
    type: () => PersonalAccessTokensWhereInput,
  })
  @ValidateNested()
  @Type(() => PersonalAccessTokensWhereInput)
  @IsOptional()
  @Field(() => PersonalAccessTokensWhereInput, {
    nullable: true,
  })
  none?: PersonalAccessTokensWhereInput;
}
export { PersonalAccessTokensListRelationFilter as PersonalAccessTokensListRelationFilter };
